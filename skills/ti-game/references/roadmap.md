...
The most expensive mistake with `ti.game` is writing code against a feature that sounds like it should be there because Phaser, Godot or Cocos has it. This file is the guard rail: everything below is **absent from upstream `main` as of 2026-08-26** (`201bcf9`, manifest `0.4.0`), with the workaround that is idiomatic today.
...
| `sprite.parent` and full transform inheritance — a child that scales, fades and flips with its parent | Partly shipped, the rest still planned (priority 7) | **Position and rotation are done**: `attachTo(target, { offsetX, offsetY, rotate })` pins a sprite to another natively, chains included, so turrets, hats, name tags and health bars need no JS. What is *not* inherited is everything else — `scale`, `opacity`, `visible`, `flipX`/`flipY`, `tintColor` — and there is no `parent` property, no hit-test or `ySort` integration. Set those on each part yourself, on the coarse tick that changes them |
