...
| `onWallLeft`, `onWallRight` | `false` | Read-only. True while the mover is pressed against that side of a matching solid this frame. Rect side push-out counts; circle/rotated-rect contacts count when their normal is mostly horizontal. Sprite and TileLayer walls are supported |
...
- **`'push'` pairs never publish ground or wall contact.** The bilateral resolver moves positions and exchanges velocity but does not update `onGround`, `onWallLeft` / `onWallRight`, `land`, or `wallhit`. A pile of balls resting on each other reports contact only with any separate `'block'` surface underneath it.
...
- **Android string maps need `c216e7f` or newer.** Earlier Android builds rejected the ordinary nested JS object behind `legend` because they required a `KrollDict`; string rows then decoded as empty cells. The current factory uses `getKrollDict()` and matches iOS. Numeric nested/flat data was unaffected.
- **Wall contact is state, not a sticky event.** `onWallLeft` / `onWallRight` reset when resolution no longer pushes the mover sideways. `wallhit` fires only on entering a wall or switching sides; use it for sound/particles, but read the flags at the instant the player requests a wall jump. A wall slide only caps positive `velocityY`; it never slows an upward wall jump.
