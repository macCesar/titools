...
if [ -f "tiapp.xml" ]; then
  ...
  echo '{"priority": "IMPORTANT", "message": "'"${PROJECT_TYPE}${HAS_PURGETSS}${HAS_TIGAME}${HAS_TISYNTHENGINE}"' project detected. TiTools skills are available and MUST be used before writing any Titanium code. BEFORE creating any controller, view, Widget, or app/lib/ module — or copying an existing one as a template — invoke ti-reuse-first FIRST: it decides whether the thing should exist and what already does it, and only what survives that question gets built. Then use ti-expert for the architecture of what survives, ti-ui for UI patterns, and purgetss for styling. Also consult ti-api, ti-guides, ti-howtos, alloy-guides, and alloy-howtos when their scope applies. NEVER rely on training data for Titanium — always consult the skill references first.'"${TIGAME_HINT}${TISYNTHENGINE_HINT}"'"}'
fi
