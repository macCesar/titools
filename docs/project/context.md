...
- **The generated `brand:` config block is reproduced three times inside the `purgetss` skill**, in `references/app-branding.md`, `references/cli-commands.md` (compressed onto fewer lines) and `references/customization-deep-dive.md`. They must stay identical to `lib/templates/purgetss.config.js.cjs` in the PurgeTSS repo, comments included. Grep for the *comment text* rather than the filename it mentions when checking them: during the 7.17.1 alignment a grep for `ic_stat_notify` found two copies, and the third — carrying the same stale comment without the filename — only surfaced on a second grep for `meta-data`. Two copies updated and one left behind is a skill that contradicts itself in the same breath.
