...
function removeLegacyEntries(dir, names, isDir = true) {
  if (!dir || !existsSync(dir)) return;
  for (const name of names) {
    const target = isDir ? join(dir, name) : join(dir, `${name}.md`);
    try {
      lstatSync(target);
...
function cleanupLegacyArtifacts(baseDir) {
  const skillsDir = getAgentsSkillsDir(baseDir);
  removeLegacyEntries(skillsDir, LEGACY_SKILLS, true);

  const agentsDir = getClaudeAgentsDir(baseDir);
  removeLegacyEntries(agentsDir, LEGACY_AGENTS, false);

  const platforms = detectPlatforms(baseDir);
  for (const platform of platforms) {
    removeLegacyEntries(platform.skillsDir, LEGACY_SKILLS, true);
  }

  if (baseDir) {
    const globalSkillsDir = getAgentsSkillsDir(undefined);
    removeLegacyEntries(globalSkillsDir, LEGACY_SKILLS, true);
    const globalAgentsDir = getClaudeAgentsDir(undefined);
    removeLegacyEntries(globalAgentsDir, LEGACY_AGENTS, false);
...
export async function updateCommand(options) {
  ...
  try {
    ...
    try {
      ...
      for (const platform of detectedPlatforms) {
        if (LEGACY_SKILLS.length > 0) {
          for (const legacy of LEGACY_SKILLS) {
            const legacyLink = join(platform.skillsDir, legacy);
            try {
              lstatSync(legacyLink);
              rmSync(legacyLink, { recursive: true, force: true });
            } catch (error) {
              if (error.code && error.code !== 'ENOENT') {
                console.log(chalk.gray(`ℹ Skipped legacy skill cleanup for ${legacy}`));
              }
            }
          }
        }
      ...
      if (isTitaniumProject(projectDir)) {
        ...
        if (hasAnyAiFile) {
          ...
          console.log(chalk.bold('▸'), 'Run in the Titanium project:', chalk.cyan('titools agents'));
        }
      } else {
        console.log(chalk.bold('▸'), 'Run in the Titanium project:', chalk.cyan('titools agents'));
      }
