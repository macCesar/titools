...
export async function agentsCommand(projectPath, options) {
  ...
  if (!hasGlobalSkills && !hasAnyLocalSkills) {
    ...
    console.log('Install titools skills first:');
    ...
    console.log(`  Global: ${agentsSkillsDir}`);
    console.log('  Local:  ./.agents/skills, ./.claude/skills, ./.gemini/skills');
    process.exit(1);
  }
  ...
  if (updated.length > 0 || removed.length > 0) {
    ...
    if (updated.length > 0) console.log(chalk.gray(`  Updated: ${updated.join(', ')}`));
    if (removed.length > 0) console.log(chalk.gray(`  Cleaned: ${removed.join(', ')}`));
  } else {
