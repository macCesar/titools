...
import { getAgentsSkillsDir, getClaudeAgentsDir, getClaudeCommandsDir } from '../config.js';
import { getSkillList, getAgentList, getCommandList } from '../cleanup.js';
...
export async function uninstallCommand(options) {
  ...
  const commandList = getCommandList().map((command) => `${command}.md`);
  ...
  const hasProjectCommands = isProject && hasAnyInDir(projectCommandsDir, commandList);
  ...
  if (hasHomeCommands || hasProjectCommands) {
    choices.push({
      name: 'Slash commands for Claude Code (`/ti-check`, `/ti-new-screen`, `/ti-audit`)',
      value: 'commands',
      checked: true,
    });
  }
  ...
  if (targets.includes('commands')) {
    ...
    const results = [];
    results.push(removeCommands(undefined));
    if (isProject) {
      results.push(removeCommands(projectDir));
    }

    const removed = results.flatMap((r) => r.removed);
    if (removed.length > 0) {
      spinner.succeed(`${removed.length} slash command${removed.length !== 1 ? 's' : ''} removed`);
      actionTaken = true;
    } else {
