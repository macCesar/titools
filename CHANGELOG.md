...
The downloader's default export now includes `fetchLatestNpmVersion`, matching its named export and the sibling AISkills API. Both repos carry the same regression test for their shared configuration and downloader surface so this kind of quiet engine drift fails in CI. Their manifest suites also inspect the actual npm tarball for generated Python bytecode, a packaging defect found in AISkills during this release.
