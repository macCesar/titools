...
```
"Generate `purgetss.ui` for my Classic app and animate a card open and closed."
```
Expect:
- Use `purgetss module`, then `require('lib/purgetss.ui')`
- Prefer `createAnimation()` with native Titanium properties
- Do not suggest `<Animation>`, utility classes, `$.*`, or Alloy files
- Include listener and draggable cleanup when the example owns either

```
"Round the launch artwork generated by `purgetss brand`, but keep every platform-masked icon untouched."
```
Expect:
- Use the v7.16 corner-radius contract only for iOS/Android legacy splashes, Feature Graphic, or LaunchLogo
- Explain the 0–50 range and shared versus per-piece precedence
- Do not pre-mask `DefaultIcon*`, launcher icons, store icons, the Android 12+ splash icon, or notification icons
